...
export class Add {
  ...
  public async profileImage(req: Request, res: Response): Promise<void> {
    ...
    const url = `https://res.cloudinary.com/dipw3x1nz/image/upload/v${result.version}/${result.public_id}`;
    const cachedUser: IUserDocument = (await userCache.updateSingleUserItemInCache(
      `${req.currentUser!.userId}`,
      'profilePicture',
      url
    )) as IUserDocument;
    socketIOImageObject.emit('update user', cachedUser); // send message to client
