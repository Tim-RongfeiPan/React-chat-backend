...
const USERNAME = 'ee23ke';
const PASSWORD = 'fdid3cc';
...
describe('SignIn', () => {
  ...
  it('should throw "Not existing user!" if username does not exist', () => {
    const req: Request = authMockRequest(
      {},
    ...
    SignIn.prototype.read(req, res).catch((error: CustomError) => {
      ...
      expect(error.statusCode).toEqual(400);
      expect(error.serializeErrors().message).toEqual('Not existing user!');
    });
  });
